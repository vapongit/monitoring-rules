  ...
  function buildBinaryExpression(head, tail) {
    return tail.reduce(function(result, element) {
      return {
        ...
        operator: String(element[1]).toLowerCase(),
        arguments: [result, element[3]]
      };
    }, head);
  }
...
Start = __ expression:Expression __ { return expression; }
...
ArrayLiteral = "[" __ elements:ElementList __ "]" {
  return {
...
MemberExpression = head:(PrimaryExpression) tail:(
    __ "." __ property:IdentifierName {
        return property.name;
    }
    / __ "." __ property:DecimalIntegerLiteral {
        return parseInt(property[0]+property[1].join(''),10);
    }
)* {
    return tail.reduce((result, element) => ({
        type: "MemberExpression",
        object: result.object ?? result,
        path: result.hasOwnProperty('path') ? result.path + '.' + element : element,
    }), head);
}
...
Arguments = "(" __ args:( ArgumentList __ )? ")" {
      return optionalList(extractOptional(args, 0));
    }

ArgumentList = head:AssignmentExpression tail:( __ "," __ AssignmentExpression )* {
...
ExponentiationExpression = head:UnaryExpression
    tail:( __ ExponentiationOperator __ ExponentiationExpression )*
    { return buildBinaryExpression(head, tail); }

ExponentiationOperator = $( "**" !"=" )

MultiplicativeExpression = head:ExponentiationExpression
...
AdditiveExpression = head:MultiplicativeExpression
...
ShiftExpression = head:AdditiveExpression
...
RelationalExpression = head:ShiftExpression
...
RelationalExpressionNoIn = head:ShiftExpression
...
EqualityExpression = head:RelationalExpression
    tail:( __ EqualityOperator __ RelationalExpression )*
    { return buildBinaryExpression(head, tail); }

EqualityExpressionNoIn = head:RelationalExpressionNoIn
...
BitwiseANDExpression = head:EqualityExpression
    tail:( __ BitwiseANDOperator __ EqualityExpression )*
    { return buildBinaryExpression(head, tail); }

BitwiseANDExpressionNoIn = head:EqualityExpressionNoIn
    tail:( __ BitwiseANDOperator __ EqualityExpressionNoIn )*
    { return buildBinaryExpression(head, tail); }

BitwiseANDOperator = $( "&" ![&=] )

BitwiseXORExpression = head:BitwiseANDExpression
    tail:( __ BitwiseXOROperator __ BitwiseANDExpression )*
    { return buildBinaryExpression(head, tail); }

BitwiseXORExpressionNoIn = head:BitwiseANDExpressionNoIn
    tail:( __ BitwiseXOROperator __ BitwiseANDExpressionNoIn )*
    { return buildBinaryExpression(head, tail); }

BitwiseXOROperator = $( "^" !"=" )

BitwiseORExpression = head:BitwiseXORExpression
    tail:( __ BitwiseOROperator __ BitwiseXORExpression )*
    { return buildBinaryExpression(head, tail); }

BitwiseORExpressionNoIn = head:BitwiseXORExpressionNoIn
    tail:( __ BitwiseOROperator __ BitwiseXORExpressionNoIn )*
    { return buildBinaryExpression(head, tail); }

BitwiseOROperator = $( "|" ![|=] )

LogicalANDExpression = head:BitwiseORExpression
    tail:( __ LogicalANDOperator __ BitwiseORExpression )*
    { return buildLogicalExpression(head, tail); }

LogicalANDExpressionNoIn = head:BitwiseORExpressionNoIn
...
LogicalORExpression = head:LogicalANDExpression
    tail:( __ LogicalOROperator __ LogicalANDExpression )*
    { return buildLogicalExpression(head, tail); }

LogicalORExpressionNoIn = head:LogicalANDExpressionNoIn
...
Expression = head:AssignmentExpression tail:( __ "," __ AssignmentExpression )* {
